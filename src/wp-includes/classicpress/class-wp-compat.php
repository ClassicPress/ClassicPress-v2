...
/*
$blocks_compatibility_level constant set the operation mode.
0: disabled
...
class WP_Compat {
	...
	private function define_helpers() {
		 ...
		 * @return void
		 */
		public function _using_block_function_row( $plugin_file, $plugin_data ) {
		 ...
		 * @return void
		 */
		public function _update_who_uses_blocks( $upgrader, $options ) {
		 ...
		 * @return void
		 */
		public function _delete_who_uses_blocks( $plugin_file ) {
			$plugins_using_blocks = get_option( 'plugins_using_blocks', array() );
			if ( array_key_exists( $plugin_file, $plugins_using_blocks ) ) {
	...
	public static function using_block_function() {
		if ( $self::blocks_compatibility_level !== 2 ) {
			return;
		}
