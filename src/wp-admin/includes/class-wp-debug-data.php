<?php
/**
 * Class for providing debug data based on a users ClassicPress environment.
 *
...
class WP_Debug_Data {
	...
	public static function debug_data() {
		...
		$info['wp-constants'] = array(
			'label'       => __( 'ClassicPress Constants' ),
			'description' => __( 'These settings alter where and how parts of ClassicPress are loaded.' ),
			'fields'      => array(
				'ABSPATH'             => array(
					'label'   => 'ABSPATH',
					'value'   => ABSPATH,
		...
		if ( is_file( ABSPATH . '.htaccess' ) ) {
			...
			// Filter away the core ClassicPress rules.
			$filtered_htaccess_content = trim( preg_replace( '/\# BEGIN ClassicPress[\s\S]+?# END ClassicPress/si', '', $htaccess_content ) );
			...
			if ( $filtered_htaccess_content ) {
				...
				/* translators: %s: .htaccess */
				$htaccess_rules_string = sprintf( __( 'Your %s file contains only core ClassicPress features.' ), '.htaccess' );
			}
	...
	/**
	 * Fetches the sizes of the ClassicPress directories: `wordpress` (ABSPATH), `plugins`, `themes`, and `uploads`.
