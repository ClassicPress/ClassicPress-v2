 ...
 * upgrade due to incompatibility with the current version of ClassicPress.
 *
 * @since 5.8.0
 * @access private
 *
	...
	foreach ( $deactivated_plugins as $plugin ) {
		if ( ! empty( $plugin['version_compatible'] ) && ! empty( $plugin['version_deactivated'] ) ) {
			$explanation = sprintf(
				...
				__( '%1$s %2$s was deactivated due to incompatibility with ClassicPress %3$s, please upgrade to %1$s %4$s or later.' ),
				$plugin['plugin_name'],
			...
			$explanation = sprintf(
				...
				__( '%1$s %2$s was deactivated due to incompatibility with ClassicPress %3$s.' ),
				$plugin['plugin_name'],
