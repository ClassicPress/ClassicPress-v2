<?php
/**
 * Class for looking up a site's health based on a user's ClassicPress environment.
 *
...
class WP_Site_Health {
	...
	public function get_test_wordpress_version() {
		...
		if ( ! is_array( $core_updates ) ) {
			...
			$result['description'] = sprintf(
				'<p>%s</p>',
				__( 'Unable to check if any new versions of ClassicPress are available.' )
			);
			...
			foreach ( $core_updates as $core => $update ) {
				if ( 'upgrade' === $update->response ) {
					...
					$result['label'] = sprintf(
						...
						__( 'ClassicPress update available (%s)' ),
						$update->version
					);
					...
					$result['actions'] = sprintf(
						...
						__( 'Install the latest version of ClassicPress' )
					);
					...
					if ( $current_major !== $new_major ) {
						...
						$result['description'] = sprintf(
							'<p>%s</p>',
							__( 'A new version of ClassicPress is available.' )
						);
					} else {
					...
					$result['label']  = sprintf(
						...
						__( 'Your version of ClassicPress (%s) is up to date' ),
						$core_current_version
					);

					$result['description'] = sprintf(
						'<p>%s</p>',
						__( 'You are currently running the latest version of ClassicPress available, keep it up!' )
					);
				}
			}
		}
	...
	public function get_test_theme_version() {
				...
				if ( $using_default_theme ) {
					...
					$result['description'] .= sprintf(
						...
						sprintf(
							...
							__( 'To enhance your site&#8217;s security, you should consider removing any themes you are not using. You should keep %1$s, the default ClassicPress theme, %2$s, your active theme, and %3$s, its parent theme.' ),
				...
				if ( $using_default_theme ) {
					...
					$result['description'] .= sprintf(
						...
						sprintf(
							...
							_n(
								'Your site has %1$d inactive theme, other than %2$s, the default ClassicPress theme, and %3$s, your active theme.',
								'Your site has %1$d inactive themes, other than %2$s, the default ClassicPress theme, and %3$s, your active theme.',
								$themes_inactive
							),
							$themes_inactive,
							$default_theme ? $default_theme->name : WP_DEFAULT_THEME,
		...
		if ( ! $has_default_theme ) {
			...
			$result['description'] .= sprintf(
				'<p>%s</p>',
				__( 'Your site does not have any default theme. Default themes are used by ClassicPress automatically if anything is wrong with your chosen theme.' )
			);
		}
	...
	public function get_test_php_default_timezone() {
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'PHP default timezone was configured by ClassicPress on loading. This is necessary for correct calculations of dates and times.' )
			),
			'actions'     => '',
		...
		if ( 'UTC' !== date_default_timezone_get() ) {
			...
			$result['description'] = sprintf(
				...
				sprintf(
					...
					__( 'PHP default timezone was changed after ClassicPress loading by a %s function call. This interferes with correct calculations of dates and times.' ),
	...
	public function get_test_sql_server() {
		...
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'The SQL server is a required piece of software for the database ClassicPress uses to store all your site&#8217;s content and settings.' )
			),
			'actions'     => sprintf(
				'<p><a href="%s" target="_blank" rel="noopener">%s <span class="screen-reader-text">%s</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a></p>',
		...
		if ( ! $this->is_acceptable_mysql_version ) {
			...
			$result['description'] .= sprintf(
				...
				sprintf(
					...
					__( 'ClassicPress requires %1$s version %2$s or higher. Contact your web hosting company to correct this.' ),
					( $this->is_mariadb ? 'MariaDB' : 'MySQL' ),
				...
				if ( is_ssl() ) {
					$result['description'] = sprintf(
						...
						sprintf(
							...
							__( 'You are accessing this website using HTTPS, but your <a href="%1$s">ClassicPress Address</a> and <a href="%2$s">Site Address</a> are not set up to use HTTPS by default.' ),
					...
					$result['description'] = sprintf(
						...
						sprintf(
							/* translators: 1: URL to Settings > General > ClassicPress Address, 2: URL to Settings > General > Site Address. */
							__( 'Your <a href="%1$s">ClassicPress Address</a> and <a href="%2$s">Site Address</a> are not set up to use HTTPS.' ),
							esc_url( admin_url( 'options-general.php' ) . '#siteurl' ),
							esc_url( admin_url( 'options-general.php' ) . '#home' )
						)
					);
				...
				if ( defined( 'WP_HOME' ) || defined( 'WP_SITEURL' ) ) {
					$result['description'] .= sprintf(
						...
						sprintf(
							...
							__( 'However, your ClassicPress Address is currently controlled by a PHP constant and therefore cannot be updated. You need to edit your %1$s and remove or update the definitions of %2$s and %3$s.' ),
	...
	public function get_test_scheduled_events() {
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'Scheduled events are what periodically looks for updates to plugins, themes and ClassicPress itself. It is also what makes sure scheduled posts are published on time. It may also be used by various plugins to make sure that planned actions are executed.' )
			),
			'actions'     => '',
	...
	public function get_test_background_updates() {
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'Background updates ensure that ClassicPress can auto-update if a security update is released for the version you are currently using.' )
			),
			'actions'     => '',
	...
	public function get_test_rest_availability() {
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'The REST API is one way that ClassicPress and other applications communicate with the server. For example, the block editor screen relies on the REST API to display and save your posts and pages.' )
			),
			'actions'     => '',
	...
	public function get_test_persistent_object_cache() {
		...
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'A persistent object cache makes your site&#8217;s database more efficient, resulting in faster load times because ClassicPress can retrieve your site&#8217;s content and settings much more quickly.' )
			),
			'actions'     => sprintf(
				'<p><a href="%s" target="_blank" rel="noopener">%s <span class="screen-reader-text">%s</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a></p>',
				esc_url( $action_url ),
